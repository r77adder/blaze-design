...
interface Props {
  ...
  /** When provided (DFY: opened from the More menu), show a back button. */
  onBack?: () => void;
}

export function CampaignsScreen({ onSettingsClick, showSkeleton, onCampaignClick, onBack }: Props) {
  return (
    <div style={{ fontFamily: font, background: 'white', minHeight: '100%', paddingBottom: 120 }}>

      {/* Header */}
      {onBack ? (
        <ToolbarHeader variant="default" leftButton="back" onLeftPress={onBack} title="Campaigns" />
      ) : (
        <ToolbarHeader
          variant="screen"
          title="Campaigns"
