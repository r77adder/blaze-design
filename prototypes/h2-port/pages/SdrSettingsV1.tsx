// ─────────────────────────────────────────────────────────────────────────
// SETTINGS V1 — snapshot of the SDR Settings page as it was *before* Gordon's
// overhaul (commit d6fd1bf, pre-#35). Revived verbatim and exposed under a
// temporary "Settings 2" tab so the old and new layouts can be compared
// side-by-side before deciding how to merge them. Delete once merged.
// Only change vs the original: CHANNEL_LABELS was renamed to SOURCE_LABELS
// upstream, so it's aliased on import below.
// ─────────────────────────────────────────────────────────────────────────
...
import { ALL_CHANNELS, SOURCE_LABELS as CHANNEL_LABELS, type Channel } from '../sdr-data';
import {
  AFTER_HOURS_OPTIONS,
  AI_VOICES,
  BOOKING_OUTPUTS,
...
const CERTAPRO_ESCALATION_TRIGGERS = [
  ...
  { id: 'complaint', label: 'Caller mentions complaint', description: 'Dissatisfied with a recent job or crew.', duringHours: 'escalate' as const, afterHours: 'escalate' as const },
  ...
  { id: 'ask-price', label: 'Asks for firm price over phone', description: 'Wants a fixed quote without an in-home estimate.', duringHours: 'escalate' as const, afterHours: 'escalate' as const },
