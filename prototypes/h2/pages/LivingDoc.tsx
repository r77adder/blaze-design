...
import styles from './LivingDoc.module.scss';
...
function ActivityGlyph({ kind }: { kind: ActivityKind }) {
  ...
  return (
    <span style={{ width: 24, height: 24, borderRadius: '50%', flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', background: s.bg, color: s.color }}>
      {kind === 'agent' ? <Stars />
        : kind === 'fathom' ? <Voice />
        : kind === 'comment' ? <Comment />
        : kind === 'posted' ? <CalendarPost />
        : kind === 'review' ? <Approvals />
        : <Send1 />}
    </span>
  );
}
...
const BRAIN_MATERIALS = [
  { name: 'CertaPro / 2026 Media', meta: 'Drive folder · 14 assets · synced', thumbs: ['https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=120&h=120&fit=crop', 'https://images.unsplash.com/photo-1581578731548-c64695cc6952?w=120&h=120&fit=crop', 'https://images.unsplash.com/photo-1599619351208-3e6c839d6828?w=120&h=120&fit=crop'] },
...
export function LivingDocRoute() {
  const { showToast } = useToast();
  ...
  // Entering client view while on the AM-only Workstream tab bounces to Overview.
  const setClientView = (v: boolean) => {
    setClientViewRaw(v);
    if (v && tab === 'work') setTab('overview');
  };
  ...
  const tabStrip = (
    <div style={{ display: 'inline-flex', gap: 6 }}>
      <TabChip selected={tab === 'overview'} onSelect={() => setTab('overview')}>Overview</TabChip>
      <TabChip selected={tab === 'meetings'} count={openThreadCount > 0 ? openThreadCount : undefined} onSelect={() => setTab('meetings')}>Meetings</TabChip>
      {!clientView && (
        <TabChip selected={tab === 'work'} count={needsTeam > 0 ? needsTeam : undefined} onSelect={() => setTab('work')}>Workstream</TabChip>
      )}
  ...
  return (
    <H2Layout topbarCenter={tabStrip} topbarRight={topbarRight}>
      <div style={{ maxWidth: 1180, margin: '0 auto', paddingBottom: 48 }}>
        ...
        {/* ── MEETINGS TAB — outline rail on the left, doc canvas right ── */}
        {tab === 'meetings' && (
