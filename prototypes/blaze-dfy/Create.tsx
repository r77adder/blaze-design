...
import { Field, FontFamilySelect, ColorSwatch, TextInput } from './ui';
...
export function Create() {
  ...
  async function create() {
    const account = await createAccount({ name, website, industry, location, poc, am: ACCOUNT_MANAGERS[amIdx], accent, colors, fonts });
    go(`/${account.id}/am/strategy`);
  }

  // Field groups — rendered on their own step and again (editable) on Confirm.
  const clientDetailsFields = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 40 }}>
      <div>
        <Heading level={3} style={{ margin: '0 0 12px' }}>Client</Heading>
      ...
      <div>
        <Heading level={3} style={{ margin: '0 0 12px' }}>Primary contact</Heading>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--light-100)' }}>
        ...
        <div style={{ maxWidth: 720, margin: '0 auto', padding: '28px 24px 40px' }}>
          ...
          {step === 'confirm' && (
            <>
              ...
              <Text variant="secondary" color="var(--dark-60)" style={{ display: 'block', margin: '4px 0 32px' }}>Looks right? Edit anything below, then create the workspace and drop into Strategy onboarding.</Text>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 40 }}>
                {clientDetailsFields}
                {brandScanFields}
              </div>
      ...
      <div style={{ flexShrink: 0, borderTop: '1px solid var(--dark-8)', background: 'var(--light-100)', padding: '16px 24px', display: 'flex', justifyContent: 'space-between', gap: 16 }}>
        ...
        {step === 'details' && <Button size="lg" onPress={() => setStep('scan')} isDisabled={!canContinue}>Continue to brand scan</Button>}
